#!/usr/bin/env python3
"""
Emit a namespaced leaf-gate TinyEngine-style C model from the Better Gate
INT8 TFLite (leaf vs non-leaf).

Same approach as Leaf Gate Model/Baseline:
  TinyEngine's GenerateSourceFilesFromTFlite fails on Flatten/Reshape, so this
  reads weights/scales from the TFLite and writes an int8→float reference
  runtime that links beside the disease MCUNet model.

Keeps the `leaf_gate_*` C API (`LEAF_GATE_CLASS_LEAF=0`,
`LEAF_GATE_CLASS_NOT_LEAF=1`) so ACLIS_IKMAL main.cpp / .cproject stay
compatible. Class 1 is the Better Gate `non-leaf` folder.
...
Run from this folder:
  ../../tinyengine/venv/bin/python emit_better_gate_c_from_tflite.py
  # or: ../../tinyengine/venv/bin/python codegen_better_gate_c.py
"""
...
def default_tflite(here: Path) -> Path:
...
TE = IKMAL / "tinyengine"
TFLITE = default_tflite(HERE)
DEFAULT_OUT = IKMAL / "ACLIS_IKMAL" / "Src" / "TinyEngine" / "codegen_leaf_gate"
OUT = DEFAULT_OUT
ACLIS_INC = IKMAL / "ACLIS_IKMAL" / "Inc"
PREFIX = "leaf_gate_"
# Short label embedded in generated C comments (override via --tflite / --label).
TFLITE_LABEL = TFLITE.name
WRITE_FIRMWARE_INC = True
...
def emit() -> None:
    ...
    # ---- header ----
    h = []
    h.append(f"/* Auto-generated Better Gate (leaf / non-leaf) from {TFLITE_LABEL} */\n")
    ...
    h.append(f"#define LEAF_GATE_CLASS_NON_LEAF 1\n")
...
"""
    ...
    nn_public = (
        "/* ACLIS Better Gate (leaf / non-leaf) — public API for main.cpp */\n"
        ...
        "#define LEAF_GATE_CLASS_NON_LEAF 1\n"
    ...
    (OUT / "Include" / "leaf_gate_nn.h").write_text(nn_public)
    if WRITE_FIRMWARE_INC:
        ACLIS_INC.mkdir(parents=True, exist_ok=True)
        (ACLIS_INC / "leaf_gate_nn.h").write_text(nn_public)

    # ---- source ----
    s = []
    s.append("/* Auto-generated Better Gate int8 reference runtime (float accum). */\n")
...
"""
    ...
    hpath.write_text(
        f"/* Auto-generated Better Gate (leaf / non-leaf) from {TFLITE_LABEL} */\n"
...
def _parse_args(argv: list[str] | None = None):
    ...
    p = argparse.ArgumentParser(
        description="Emit namespaced Better Gate C from an INT8 TFLite model."
    )
    p.add_argument(
        "--tflite",
        type=Path,
        default=None,
        help=(
            "Path to full-int8 TFLite (default: "
            "aclis_leaf_gate_96x_alt_full_int8.tflite next to this script or "
            "under leaf_gate_output_alt/)"
        ),
    )
    p.add_argument(
        "--label",
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Codegen output directory (default: "
            "ACLIS_IKMAL/Src/TinyEngine/codegen_leaf_gate). "
            "A custom path does not overwrite ACLIS_IKMAL/Inc/leaf_gate_nn.h."
        ),
    )
...
if __name__ == "__main__":
    ...
    if args.out is not None:
        ...
        WRITE_FIRMWARE_INC = OUT == DEFAULT_OUT.resolve()
